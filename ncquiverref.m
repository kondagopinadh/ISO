...
if refvec
 ...
 ht=text(xstart,yp1+pady,reftext,'Visible','off','FontSize',16,'Fontweight','bold',...
        'VerticalAlignment','Bottom','HorizontalAlignment','Right');
 textextent=get(ht,'Extent');
 % Draw patch over area of vector key 
 xl=textextent(1)-padx*(-8);
 xr=xp2;
 yb=yp1;
 yt=textextent(2)+textextent(4)+pady;
 hp=patch([xl; xl; xr; xr],[yb; yt; yt; yb],[2; 2; 2; 1],'w',...
          'LineWidth',1);
 uistack(hp,'top');
 % Redraw reference text on top of patch
 ht=text(xstart,(yb+yt)/2,2.1,reftext,'FontSize',16,'Fontweight','bold',...
